...
public class YxcPhotoActivity extends AbActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setAbContentView(R.layout.activity_yxc);
        ...
        mYxc = (Yxc) intent.getSerializableExtra("yxc");

        mView_dwmc.setText(mYxc.getDwmc());
        mView_kcry.setText(application.getUser().getFzr());

        AbTitleBar mAbTitleBar = this.getTitleBar();
        mAbTitleBar.setTitleText("登记启动");
        ...
        if (AbStrUtil.isEmpty(photo_dir)) {
            MsgUtil.sendMsgTop(YxcPhotoActivity.this, Constant.MSG_ALERT, "存储卡不存在");
        } else {
            ...
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                if (AbStrUtil.isEquals(mPhotoList.get(0),cameraPath)) {
                    if (application.mLocation.isSucess() && !AbStrUtil.isEmpty(application.mLocation.getAddress())) {
                        ...
                        MsgUtil.sendMsgTop(YxcPhotoActivity.this, Constant.MSG_CONFIRM, "未取得定位信息,请稍后再试...");
                    }
                } else {
                    showDiag(position);
                }
        ...
        addBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                final AbLoadDialogFragment mDialogFragment = AbDialogUtil.showLoadDialog(YxcPhotoActivity.this, R.drawable.list_load, "请稍候...", AbDialogUtil.ThemeLightPanel);
                if(mPhotoList.get(0).indexOf("http://")==-1) {
                    uploadFile(mPhotoList.get(0));
                }else {
                    commit();
                }
...
//        if(AppUtil.GpsIsOPen(this)){
//            AbLogUtil.i("xxxx","no gps");
...
//        }
    ...
    private void doPickPhotoAction() {
        ...
        if (status.equals(Environment.MEDIA_MOUNTED)) {
            ...
            MsgUtil.sendMsgTop(YxcPhotoActivity.this, Constant.MSG_ALERT, "没有可用的存储卡");
        }
    }
    ...
    protected void doTakePhoto() {
        try {
            ...
            MsgUtil.sendMsgTop(YxcPhotoActivity.this, Constant.MSG_ALERT, "未找到系统相机程序");
        }
    }
    ...
    private void showDiag(final int position) {
        ...
        AlertView mAlertView = new AlertView("操作菜单", null, "取消", null,
                ...
                YxcPhotoActivity.this, AlertView.Style.ActionSheet, new com.bigkoo.alertview.OnItemClickListener() {
            @Override
            ...
            public void onDismiss(Object o) {
                switch (choose) {
                    case 0:
                        ...
                        intent.setClass(YxcPhotoActivity.this, ShowPhotoActivity.class);
                        startActivity(intent);
                        break;
                    case 1:
                        new AlertView("请再次确认", "确认删除这张照片 ?", "取消", new String[]{"确定"}, null, YxcPhotoActivity.this, AlertView.Style.Alert, new com.bigkoo.alertview.OnItemClickListener() {
                            @Override
                            public void onItemClick(Object o, int position) {
                                if (position == 0) {
                                    deleteFile(position);
    ...
    private boolean checkProject() {
        if (mPhotoList.size() < 1 || AbStrUtil.isEquals(mPhotoList.get(0),cameraPath)) {
            MsgUtil.sendMsgTop(YxcPhotoActivity.this, Constant.MSG_CONFIRM, "请拍摄照片");
            return false;
        }
        return true;
    }
    ...
    private void savePhotoListLocation(String path) {
        ...
        mYxc.setLatitude(latitude);
        mYxc.setLontitude(lontitude);
        mYxc.setAddress(address);
        mYxc.setLocationdescribe(locationdescribe);
    ...
    public void uploadFile(final String path) {
        ...
        mAbHttpUtil.post(url, params, new AbStringHttpResponseListener() {
            ...
            public void onFailure(int statusCode, String content,
                ...
                MsgUtil.sendMsgTop(YxcPhotoActivity.this, Constant.MSG_ALERT, error.getMessage());
            }
            ...
            public void onFinish() {
                //上传图片 完毕 上传工程
                    AbDialogUtil.removeDialog(YxcPhotoActivity.this);
                    if (mAlertDialog != null) {
                        mAlertDialog.dismiss();
                        mAlertDialog = null;
                    }
    ...
    private void commit() {
        String gson = AbJsonUtil.toJson(mYxc);
        AbSoapParams params = new AbSoapParams();
        params.put("action", "add");
        params.put("gson", gson);
        WebServiceUntils.call(YxcPhotoActivity.this, Constant.Modify_YXC, params, 10000, false, "", new WebServiceUntils.webServiceCallBack() {
            @Override
            public void callback(Boolean aBoolean, String result) {
                if (aBoolean && AbStrUtil.isEquals(result, "1")) {
                    MsgUtil.sendMsgTop(YxcPhotoActivity.this, Constant.MSG_INFO, "登记启动成功!");
                    isCommit = true;
                    ...
                    MsgUtil.sendMsgTop(YxcPhotoActivity.this, Constant.MSG_ALERT, "上传失败,请重试");
                    addBtn.setEnabled(true);
                    //下载完成取消进度框
                    if (mAlertDialog != null) {
                        mAlertDialog.dismiss();
    ...
    private void initGPS() {
        ...
        if (!locationManager
            ...
            new AlertView("请确认", "GPS未打开,请打开GPS开关!","取消", new String[]{"确定"}, null, YxcPhotoActivity.this, AlertView.Style.Alert, new com.bigkoo.alertview.OnItemClickListener() {
                @Override
                public void onItemClick(Object o, int position) {
                    if (position == 0) {
                        Intent intent = new Intent(
